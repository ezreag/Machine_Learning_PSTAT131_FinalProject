...
# X_train_transformed = sc_X_train.fit_transform(X_sdsurf_train)
# y_train_transformed = sc_y_train.fit_transform(y_sdsurf_train)

nusvr_model = NuSVR()
nusvr_model = nusvr_model.fit(X_sdsurf_train, y_sdsurf_train)
nusvr_model_fitparams = nusvr_model.get_params

nusvr_model_R2 = nusvr_model.score(X_sdsurf_test, y_sdsurf_test)

y_prediction = nusvr_model.predict(X_sdsurf_test)

nusvr_model_predR2 = r2_score(y_sdsurf_test, y_prediction) # same as ridgecv_model_score
nusvr_model_predMSE = mean_squared_error(y_sdsurf_test, y_prediction)

print("Estimated Parameters: ", nusvr_model_fitparams, "\nBest Estimated R^2 Score: ", nusvr_model_R2, "\n\nR^2 Prediction Error Score: ", nusvr_model_predR2, "\nMSE Prediction Error Score: ", nusvr_model_predMSE, "\nRMSE Prediction Error Score: ", sqrt(nusvr_model_predMSE))
```
...
scaler_X = StandardScaler().fit(X_sdsurf_train)
scaler_y = StandardScaler().fit(y_sdsurf_train.values)
X_train_transformed = scaler_X.transform(X_sdsurf_train)
y_train_transformed = scaler_y.transform(y_sdsurf_train)
X_test_transformed = scaler_X.transform(X_sdsurf_test)
y_test_transformed = scaler_y.transform(y_sdsurf_test)

svr_model = LinearSVR(random_state=seed_val)
svr_model = svr_model.fit(X_train_transformed, y_train_transformed)
svr_model_fitparams = svr_model.get_params

svr_model_R2 = svr_model.score(X_test_transformed, y_test_transformed)

y_prediction = svr_model.predict(X_test_transformed)

svr_model_predR2 = r2_score(y_test_transformed, y_prediction) # same as ridgecv_model_score
svr_model_predMSE = mean_squared_error(y_test_transformed, y_prediction)
...
```{python eval=F}
print(X_sdsurf_train)
print(y_sdsurf_train.values.reshape())
```
...
KNN_model_predR2 = KNN_model.score(X_sdsurf_test, y_sdsurf_test)
KNN_model_predMSE = mean_squared_error(y_sdsurf_test, KNN_y_prediction)
...
HGBoost_model_predR2 = HGBoost_model.score(X_sdsurf_test, y_sdsurf_test)
HGBoost_model_predMSE = mean_squared_error(y_sdsurf_test, HGBoost_y_prediction)
...
```{python class.source="fold-show"}
RidgeCV_list = [["MSE Estimate", 1.2065245693118951], ["RMSE Estimate", 1.0984191227905198], ["R^2 Prediction Error", 0.6491233448758329], ["MSE Prediction Error", 2.6320537070147103], ["RMSE Prediction Error", 1.6223605354589683]]
...
LinearSVR_list = []
# Linear Support Vector Regressor Model 
## Run-Time: N/A MINUTES
##
## Estimated Parameters:  NA
## Best Estimated R^2 Error Score:  NA 
## 
## R^2 Prediction Error Score:  NA 
## MSE Prediction Error Score:  NA 
## RMSE Prediction Error Score:  NA

KNNRegression_list = [["Estimated Parameters", "<bound method BaseEstimator.get_params of KNeighborsRegressor(algorithm='kd_tree', leaf_size=55000)>"], ["R^2 Prediction Error", 0.656779427108612], ["MSE Prediction Error", 2.5746226430561667], ["RMSE Prediction Error", 1.6045630692048745]]
...
## Number of Trees Build at each Iteration:  1 
## Scores at each Iteration on the Training Data:  NA
## Scores at each Iteration on the Validation Data: NA
##
